...
const ProductDetails = () => {
    ...
    useEffect(()=>{
        fetch(`http://localhost:4000/orders/email/productName?email=${email}&productName=${name}`)
        .then(res=>res.json())
        .then(data=>{
            console.log(data);
            if(data?.orderQuantity ===null){
                ...
                setOrderQuantity(parseInt(data?.orderQuantity) + minOrderQuantity);
                
            }
            setAvailableQuantity(availableQuantity-orderQuantity);
        })
    },[email,name]);
    console.log(orderQuantity,availableQuantity);
    ...
    console.log(orderQuantity,pricePerUnit);
    const handlePurchase = async() =>{
        ...
        console.log(order);
        SetNewOrder(order);
        await fetch(`http://localhost:4000/orders?email=${email}&productName=${name}`, {
            method: 'PUT',
            headers: {
