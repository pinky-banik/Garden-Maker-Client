      ...
      >
        {blogs.map((blog, index) => (
          <SwiperSlide key={index}>
            ...
            <div
              className="max-w-sm overflow-hidden text-start cursor-pointer bg-gray-100 p-5"
              onClick={() => handleRoute(blog._id)}
            >
              <div>
                <div>
                  <div
                    style={{ backgroundImage: `url('${blog.img}')` }}
                    className="group w-full h-72 bg-cover bg-center   justify-center"
                  >
                    <div
                      className="bg-secondary w-full h-full flex justify-center items-center bg-opacity-0 hover:bg-opacity-80
                        trasition duration-500 "
                    >
                      <button
                        onClick={() => handleRoute(blog._id)}
                        className="hidden w-36 h-12 group-hover:block btn btn-primary "
                      >
                        Learn more
                      </button>
                    </div>
